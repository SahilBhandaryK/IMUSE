...
def deg2ind(deg_list):  # Convert angle in degree to index of Angle_Sweep   Updated on 15th Nov. 2020
    ...
    index_list = np.array([])
    for deg in deg_list:
        index_list = np.append(index_list, (N_Angle-1)
                               * (deg-theta_start)/(theta_end - theta_start))
    index_list = index_list.astype(int)
    return index_list


# Range is from X0 to source
def MixGen_att(SigVec, Angle, M, fs_true, SNR, factor_ups, vp, d, Range):
    ...
    for i in range(M):
        for j in range(K):
            ...
            X[i, :] += alpha * Tshift_Ups(SigVec[j, :], -i*d
                                          * np.cos(Angle[j]*np.pi/180)/vp, fs_true, factor_ups)
    ...
    noiseVec = np.random.normal(0, np.sqrt(
        SigMaxVar/(10**(SNR/10))), size=(M, len(SigVec[0, :])))  # Noise in time domain is real
...
def eval_SI_SDR(s_ref, s_est):
    s_tilda = np.vdot(s_ref, s_est)*s_ref / \
                      np.square(np.linalg.norm(s_ref, ord=2))
...
def Find_AlignRotAng(Angle_est, index_want):
    if len(Angle_est) > 2:
        k = index_want
        Theta_align = np.average(
            np.array(list(itertools.combinations(Angle_est, 2))), axis=1)
        Q = np.zeros((len(Theta_align)))
        for i in range(len(Theta_align)):
            Q_list = np.abs(np.abs(AoA_clip(
                Angle_est - Theta_align[i]))-np.abs(AoA_clip_number(Angle_est[k] - Theta_align[i])))
